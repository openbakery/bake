...
public extension Confirmation {
	...
	static func wait<T: Sendable, S: Sendable>(fulfillmentSource: T, actionSource: S, fulfillmentClosure: @escaping @Sendable (T) -> Bool, actionClosure: @escaping @Sendable (S) -> Void) async {
	...
	static func wait(fulfillmentClosure: @Sendable @escaping () -> Bool, actionClosure: @Sendable @escaping () -> Void) async {
		if fulfillmentClosure() {
			return
		}
		await confirmation("called") { confirmation in
